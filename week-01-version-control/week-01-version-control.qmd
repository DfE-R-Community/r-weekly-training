...
-   Usually, next to a `origin/my-branch` you'll also have `my-branch`.
    This indicates the status of your *local* version of the branch.
    These may not be in the same place though. If not, it will either be
    because you're ahead of the remote version (fixed with `git push`)
    or the remote version is ahead of you (fixed with `git pull`).
...
-   The branch being merged *into* is
    `DfE-R-Community/r-weekly-training`

-   The branch being merged *from* is
    `you/solutions/week-01-version-control`

-   You have assigned someone to review your code (here wklimowicz is
